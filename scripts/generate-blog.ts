...
function getTodaysTopic(): typeof KEYWORD_TOPICS[0] {
  const dayOfYear = Math.floor(
    (Date.now() - new Date(new Date().getFullYear(), 0, 0).getTime()) /
      (1000 * 60 * 60 * 24)
  );
  return KEYWORD_TOPICS[dayOfYear % KEYWORD_TOPICS.length];
}

async function generateBlogPost(topic: typeof KEYWORD_TOPICS[0]): Promise<string> {
...
9. Mention LocalLeads naturally once or twice — as a tool you've seen people use, not as an advertisement. Make it feel like a recommendation from a friend, not a sales pitch.
10. Each post must have a specific, actionable section where the reader learns something real — a script they can use, a number they can charge, a specific step they can take today.
11. The post should feel like it gives so much value that the reader bookmarks it. Not a sales page. A genuinely useful piece.
12. Write 1200-1500 words. No more. No less.
13. Format: H1 title, then sections with H2 subheadings, written in flowing paragraphs.
14. End with one real, honest call to action — not corporate language.`;

  const userPrompt = `Write a blog post targeting this keyword: "${topic.keyword}"

City focus: ${topic.city}
Target reader: A freelance web designer in India who is struggling to find clients

The post must:
- Rank for the keyword naturally — weave it in, don't stuff it
- Give genuine, specific value that the reader can act on today
- Mention real scenarios from Indian cities
- Include one specific pitch script or actionable template
- Mention LocalLeads (localleads.sahajta.com) once as a tool that helps find businesses without websites — naturally, as a recommendation, not an ad

Output format: Start with the H1 title on the first line, then write the full article in markdown.`;
...
async function main() {
  ...
  if (fs.existsSync(filepath)) {
    console.log("Blog post already generated today:", filename);
    return;
  }

  console.log("Generating blog post for:", topic.keyword);
...
description: "${title.replace(/"/g, "'")} — practical guide for Indian web freelancers and designers looking for clients."
---
